package jumpstart.web.components.examples.ajaxgracefulcrud;
 ...
 * {@link jumpstart.web.components.examples.ajaxgracefulcrud.PersonEditor#CANCEL_CREATE},
 * {@link jumpstart.web.components.examples.ajaxgracefulcrud.PersonEditor#SUCCESSFUL_CREATE}(Long personId),
 * {@link jumpstart.web.components.examples.ajaxgracefulcrud.PersonEditor#FAILED_CREATE},
 * {@link jumpstart.web.components.examples.ajaxgracefulcrud.PersonEditor#TO_UPDATE}(Long personId),
 * {@link jumpstart.web.components.examples.ajaxgracefulcrud.PersonEditor#SUCCESSFUL_UPDATE}(Long personId),
 * {@link jumpstart.web.components.examples.ajaxgracefulcrud.PersonEditor#FAILED_UPDATE}(Long personId),
 * {@link jumpstart.web.components.examples.ajaxgracefulcrud.PersonEditor#CANCEL_CONFIRM_DELETE}(Long personId),
 * {@link jumpstart.web.components.examples.ajaxgracefulcrud.PersonEditor#SUCCESFUL_DELETE}(Long personId),
 * {@link jumpstart.web.components.examples.ajaxgracefulcrud.PersonEditor#FAILED_DELETE}(Long personId).
 */
...
public class PersonEditor {
	...
	@Component(id = "confirmDeleteForm")
	private Form _confirmDeleteForm;
	...
	Object onDelete(Long personId, Integer personVersion) {
		...
		// Else, (JavsScript disabled) user has pressed Delete..., but not yet confirmed so go to a confirmation screen.
	...
	// Component "confirmDeleteForm" triggers the PREPARE event when it is rendered or submitted

	void onPrepareFromConfirmDeleteForm() {
		_mode = Mode.CONFIRM_DELETE;
		if (_person == null) {
			// Get objects for the form fields to overlay.
			_person = getPersonService().findPerson(_personId);
	...
	// Component "confirmDeleteForm" triggers the VALIDATE_FORM event when it is submitted

	void onValidateFormFromConfirmDeleteForm(Long personId, Integer personVersion) {

		if (_confirmDeleteForm.getHasErrors()) {
			// We get here only if a validator detected an error and javascript is disabled in the browser.
			return;
		}
		...
		if (_person != null) {
			...
			if (_demoModeStr != null && _demoModeStr.equals("true")) {
				_confirmDeleteForm.recordError("Sorry, but Delete is not allowed in Demo mode.");
			}
			else {
				...
				try {
					if (!_person.getVersion().equals(personVersion)) {
						_confirmDeleteForm
								.recordError("Cannot delete person because has been updated or deleted since last displayed. Please refresh and try again.");
					}
					else {
						getPersonService().deletePerson(_person);
					}
					...
					// Display the cause. In a real system we would try harder to get a user-friendly message.
					_confirmDeleteForm.recordError(ExceptionUtil.getRootCause(e));
				}
			}
		}
	...
	// Component "confirmDeleteForm" triggers SUCCESS or FAILURE when it is submitted, depending on whether
	// VALIDATE_FORM records an error

	Object onSuccessFromConfirmDeleteForm() {
		// Trigger new event "successfulDelete" (which in this example will bubble up to the page).
		// When the event has been handled, Tapestry will call my callback with the result.
		_componentResources.triggerEvent(SUCCESFUL_DELETE, new Object[] { _person.getId() }, getMyCallback());
	...
	Object onFailureFromConfirmDeleteForm() {
		// Trigger new event "failedDelete" (which in this example will bubble up to the page).
		// When the event has been handled, Tapestry will call my callback with the result.
		_componentResources.triggerEvent(FAILED_DELETE, null, getMyCallback());
	...
	public boolean isModeConfirmDelete() {
		return _mode == Mode.CONFIRM_DELETE;
	}
